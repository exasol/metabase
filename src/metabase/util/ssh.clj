...
(defn start-ssh-tunnel
  ...
  (let [client (doto (SshClient/setUpDefaultClient)
                 (.start))

        conn-future (.connect ^SshClient client ^String tunnel-user ^String tunnel-host ^Integer tunnel-port)
        ...
        input-port (.getPort ^SshdSocketAddress (.getLocalAddress ^PortForwardingTracker tracker))]
    (log/error (u/format-color 'cyan "creating ssh tunnel %s@%s:%s -L %s:%s:%s" tunnel-user tunnel-host tunnel-port input-port host port))
...
(defn include-ssh-tunnel
  ...
  (if (use-ssh-tunnel? details)
    (let [[_ proto host] (re-find #"(.*://)?(.*)" (:host details))
          [client session tracker] (start-ssh-tunnel (assoc details :host host)) ;; don't include L7 protocol in ssh tunnel
          tunnel-entrance-port (.getPort ^SshdSocketAddress (.getLocalAddress ^PortForwardingTracker tracker))
...
(defn close-tunnel
  ...
  ([details]
   (when (use-ssh-tunnel? details)
     (close-tunnel (:tunnel-client details) (:tunnel-session details) (:tunnel-tracker details))))
  ([client session tracker]
   (.close ^Channel tracker)
   (.close ^ClientSession session)
   (.stop ^SshClient client)))
...
(defn with-ssh-tunnel*
  ...
  (if (use-ssh-tunnel? details)
    (let [details-with-tunnel (include-ssh-tunnel details)]
      (try
        (log/error (u/format-color 'cyan "<< OPENED SSH TUNNEL >>"))
        (f details-with-tunnel)
        (catch Exception e
          (throw e))
        (finally
          (close-tunnel details-with-tunnel)
          (log/error (u/format-color 'cyan "<< CLOSED SSH TUNNEL >>")))))
    (f details)))
